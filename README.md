...
- Base64 文本（标准 Base64 和 URL-safe Base64）
- 10 位 Unix 时间戳（秒）
- 13 位 Unix 时间戳（毫秒）
- JWT（本地解码 header / payload，不验证签名）
...
### 1. Base64 自动识别与解码
- 选中合法标准 Base64 或 URL-safe Base64 文本
...
### 3. JWT 本地解码
- 选中标准三段式 JWT
- 本地解码并格式化展示 header 与 payload JSON
- 明确提示签名未验证，不做远程请求
- 支持复制原文和完整解析结果
...
1. 打开任意普通网页
2. 选中一段 Base64、JWT 或 10/13 位时间戳
...
- 确认 manifest 无报错
- 确认核心能力仍然可用
- 标准 Base64 和 URL-safe Base64 解析正常
- JWT 解码正常，并明确显示签名未验证
- 10/13 位时间戳解析正常
- 选中文本右键菜单触发解析正常
- “复制原文”和“复制结果”功能正常
...
### 简介
SelectLens helps you inspect selected text directly on webpages. Instantly decode Base64 and JWTs locally, convert 10-digit or 13-digit Unix timestamps into readable time, and copy results with one click. Built for developers, testers, and anyone who works with encoded or time-based data online.
...
- Base64 和 JWT 解码结果要求可被解析为 UTF-8 文本，二进制内容不会展示
- JWT 仅解码 header / payload 并提示签名未验证，不做签名校验
- 在 `chrome://` 等受限页面中无法读取选区
- input / textarea 内的悬浮卡片位置使用输入框级别的简化定位

## 后续可扩展方向

- URL Decode / JSON Format 等更多调试工具
- 悬浮卡片位置和动效进一步优化
